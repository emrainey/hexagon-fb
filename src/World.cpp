...
// class constructor
World::World(int num, string *start_view)
{
    debug->info(DEBUG_TRACE,"World((Views)%i)\n",num);
    num_total_views = num;
    ...
    for (int i=0; i < num_total_views; i++)
        addView();
    
    selection.state = SELECTION_OLD;
...
void World::addView(void)
{
    ...
    else
    {
        ...
        debug->info(DEBUG_INTERNAL,"new view @ %s\n",v->position.print());
    }
...
World::~World()
{
    list<View *>::iterator lit = view_list.begin();
...
{
    debug->info(DEBUG_TRACE,"+World::choose(0x%08x)\n",name);
    debug->info(DEBUG_MENU,"selection.state=%i\n",selection.state);
    ...
    while (lit != view_list.end())
    {
        ...
        {
            focus = (*lit);
            debug->info(DEBUG_TRACE,"-World::choose(0x%08x) [2]\n",name);
            return;
        }
        lit++;
    }
    ...
    if (selection.state == SELECTION_CONTEXT_MENU)
    {
        debug->info(DEBUG_MENU,"menu is %snull\n",(menu?"not ":""));
...
{
    debug->info(DEBUG_TRACE,"World::placeView(%i)\n",v);
    debug->info(DEBUG_INTERNAL,"\tnext_fill=%i\n",next_fill);

    // create the vector to the new View
    Vector *vec = direction(next_fill);
    debug->info(DEBUG_INTERNAL,"\tvec->abs()=%lf\n",vec->abs());
    ...
    if (unfull->neighbors[next] != NULL)
    {
        ...
        debug->info(DEBUG_INTERNAL,"\tv->neighbor[%i]=%p\n",inv,unfull->neighbors[next]);
    }
...
Vector *World::direction(int direction)
{
    if (SIDES != 6)
    {
        ...
        debug->info(DEBUG_INTERNAL,"degrees=%lf\n",degrees);
        ...
        debug->info(DEBUG_INTERNAL,"x=%lf\n",x);
        debug->info(DEBUG_INTERNAL,"y=%lf\n",y);
        debug->info(DEBUG_INTERNAL,"direction=%i\n",direction);
        Vector g(1.5,1,0);
        debug->info(DEBUG_INTERNAL,"g.abs=%lf\n",g.abs());
...
int World::getNextPosition(int view_num)
{
    ...
    else
    {
        ...
        // the intra_ring_offset is the offset from the beginning of the current ring.
        // so if we are in the 2nd ring at 12 (which starts at 7, zero based) we
        // substract 1 to account for the root tile then subtract the number of 
    ...
    debug->info(DEBUG_ANY,"Something is REALLY screwed up!\n");
    return 0;
}
...
{
    list<Light *>::iterator lit = lights.begin();
