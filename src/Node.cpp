...
Node::Node()
{
    ...
    name = "A Tile";
    path = "Somewhere";
    description = new string("A default Node() instance");
...
Node::Node(string n, string p, string *d, Node_Type_e nt, Model_Type_e mt, int select_name)
{
...
void Node::renderDescription(bool extra)
{
    ...
    {
        Vector text = position + *(new Vector(0,0,1));
        if (extra == true)
        {
...
bool Node::choose(int name)
{
    ...
    {
        debug->info(DEBUG_SELECTION,"%s has been selected!\n",this->name.c_str());
        return true;
    }
}
