 ...
 */
string Win32FS::translateToInternal(std::string path) {
 ...
 */
string Win32FS::translateToExternal(std::string path) {
    path.erase(0, 1);
    for (int i = 0; i < path.length(); i++)
 ...
 * initialized Nodes.
 */
void Win32FS::populateNodeList(Layer *layer, std::string path) { /* TODO (#1#): Implement Win32FS::populateNodeList() */
}

// This function returns, as std::strings, the contents of this directory.
std::list<std::string> *Win32FS::getDirectoryList(std::string path) {
    std::list<std::string> *bob = new std::list<std::string>();

    if (path == ROOT_PATH) {
        // they want the root drive layout
        // the list will contains std::strings like
        // "C:\","D:\","X:\","Z:\"
...
string *Win32FS::getMetaData(std::string path, std::string name) {
    if (path == ROOT_PATH) {
        char logicaldrives[LD_MAX];
...
Model_Type_e Win32FS::getModelType(std::string path, std::string name) {
    if (path == ROOT_PATH) {
